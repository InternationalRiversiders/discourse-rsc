...
module DiscourseRsc
  module Views
    def self.position(position)
      ...
      {id:position.id,instrument_id:instrument.id,symbol:instrument.symbol,side:position.side,leverage:position.leverage,quantity:Amount.format(position.quantity_units),average:Amount.format(position.average_units),margin:Amount.format(margin),pnl:profit && Amount.format(profit),equity:equity && Amount.format(equity),maintenance:Amount.format(margin/4),break_even:Amount.format(TradingRules.break_even(position,instrument.fee_bps)),risk:equity && (equity<=margin/4 ? 'liquidation' : equity<=margin/2 ? 'high' : 'normal'),valuation_basis:valuation[:basis],valuation_at:valuation[:at],take_profit:position.take_profit_units && Amount.format(position.take_profit_units),stop_loss:position.stop_loss_units && Amount.format(position.stop_loss_units),hold_until:position.hold_until,liquidation:Risk.liquidation(position)}
    end
