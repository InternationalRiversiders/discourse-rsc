  ...
  class Rewards
    ...
    def self.preview(date)
      ...
      ids.filter_map do |user_id|
        ...
        next unless user && Access.member?(user)
        score = [posts.fetch(user_id, 0) + (visits.include?(user_id) ? 1 : 0), 10].min
        { user_id: user_id, username: user.username, score: score, paid: Command.exists?(key: "daily_reward:#{user_id}:daily-#{date}"), frozen: Account.wallet_snapshot(user_id).status != "active" }
      end
